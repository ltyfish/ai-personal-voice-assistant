...
const updater = read("desktop/src/main/updater.ts");
...
assert(/desktop:getUpdateStatus/.test(main), "main process must expose update status IPC");
...
assert(/onUpdateStatus/.test(preload), "preload must expose update state changes");
assert(/Restart and update/.test(app), "renderer must offer confirmed restart after download");
